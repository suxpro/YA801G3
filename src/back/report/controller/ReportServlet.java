	...
	protected void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if (contentType != null
			...
			multi = new MultipartRequest(req, getServletContext().getRealPath(
					"/back/report/images"), 5 * 1024 * 1024, "UTF-8");
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back/report/select_page.jsp");
				...
				String url = "/back/report/listOneReport.jsp";
		...
		if ("getOne_For_Update".equals(action)) { // 來自listAllEmp.jsp的請求
			...
			try {
				...
				/*************************** 3.查詢完成,準備轉交(Send the Success view) ************/
				req.setAttribute("reportVO", reportVO); // 資料庫取出的empVO物件,存入req
				String url = "/back/report/update_report_input.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);// 成功轉交
		...
		if ("update".equals(action)) { // 來自update_report_input.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					...
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back/report/update_report_input.jsp");
					failureView.forward(req, res);
					return; // 程式中斷
				}
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("reportVO", reportVO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back/report/addReport.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				String url = "/back/report/listAllReport.jsp";
