				...
				<c:forEach var="message" items="${errorMsgs}">
					...
					<table class="table table-hover " style="white-space: nowrap;">
						<tr class="info">
							...
							<th style="text-align: center">聯絡方式</th>
						...
						<c:forEach var="ordVO" items="${list}" begin="<%=pageIndex%>"
							...
							<tr align='center' valign='middle'>
								...
								<td rowspan="2"><img width="100" height="100"
								...
								<td rowspan="2">${ordVO.ten_date       }<br> ~ <br>
									${ordVO.exp_date       }
								</td>
								...
								<td align='left'>Mail : <a
									href="mailto:<%=lesVO.getMmail()%>"><%=lesVO.getMmail()%></a><br>
														...
														<table border='1' bordercolor='#CCCCFF' align='center'>
															...
															<tr>
																...
																<td align='left'><input type="text"
																	name="nexp_date" id="nexp_date<%=count%>"
																	id="nexp_date"></td>
															</tr>
															<tr>
																<td>延展天數</td>
															...
															<tr>
																<td>租金/天</td>
																<td align='left'><input type="text"
																	name="unit_price" id="unit_price<%=count%>"
																	value="${rentVO.unit_price}" readonly='readonly'></td>
															</tr>
															<tr>
																<td>需補差額</td>
																<td align='left'><input type="text"
																	name="dif_price" id="dif_price<%=count%>" value="0"
							...
							$("#nexp_date<%=count%>").change(
									function() {
										...
										$("#dif_price<%=count%>").val(dif_price);
											...
											<td>
												<button class="btn btn-primary" id="rec_com<%=count%>"
												...
												<form method="post"
													...
													<input type="hidden" name="sta" value="REC_COM"> <input
														type="hidden" name="reqURL"
														value="/front/ord/tenOrdList.jsp"> <input
														id="receive<%=count%>" type="submit" value="收貨">
												</form>
											</td>
											<td>
												...
												<div id="dialog-formB<%=count%>" title="給予出租會員評價">
													...
													<form method="post"
														action="<%=request.getContextPath()%>/front/ord/ord.do">
														<label>評價等級:</label> &nbsp;<input type="radio" name="ases"
															value=2>&nbsp;特優 &nbsp;<input type="radio"
															name="ases" value=1>&nbsp;良好 &nbsp;<input
															type="radio" name="ases" value=0>&nbsp;普通 &nbsp;<input
															type="radio" name="ases" value=-1>&nbsp;極差<br>
														<h4>輸入此次交易的意見.</h4>
														<textarea name="ases_ct" rows="4" cols="35"
															maxlength="100"></textarea>
														<input type="hidden" name="ord_no" value="${ordVO.ord_no}">
														<input type="hidden" name="action" value="update_ases">
														<input type="hidden" name="role" value="ten"> <input
															type="hidden" name="reqURL"
															value="/front/ord/tenOrdList.jsp"> <input
															id="ases<%=count%>" type="submit">
													</form>
												</div>
											</td>
											<td>
												<button id="cc_ord<%=count%>" class="btn btn-primary">取消</button>
												<div id="dialog-formC<%=count%>"
													title="取消訂單[${ordVO.ord_no}]">
													<p>請輸入取消訂單的原因.</p>
													<form method="post"
														action="<%=request.getContextPath()%>/front/ord/ord.do">
														<input type="hidden" name="ord_no" value="${ordVO.ord_no}">
														...
														<textarea name="ord_cc_cause" rows="4" cols="35"
															maxlength="100"></textarea>
														<input id="cancel<%=count%>" type="submit">
													</form>
												</div>
											</td>
											<!-- 					<td> -->
											<%-- 					<button id="rep_ten<%=count%>" class="btn btn-primary">檢舉</button> --%>
											<%-- 					<div id="dialog-formD<%=count%>" title="檢舉租物[${rentVO.rent_name}]"> --%>
											<!-- 						<p>請輸入檢舉租物的原因.</p> -->
											<!-- 						<form method="post" -->
											<%-- 							action="<%=request.getContextPath()%>/front/report/report.do"> --%>
											<%-- 							<input type="hidden" name="ord_no" value="${ordVO.ord_no}"> --%>
											<!-- 							<input type="hidden" name="action" value="report"> <input -->
											<!-- 								type="hidden" name="role" value="ten"> -->
											<!-- 							<textarea name="rep_cause" rows="4" cols="35" maxlength="100"></textarea> -->
											<%-- 							<input id="report<%=count%>" type="submit"> --%>
											<!-- 						</form> -->
											<!-- 					</div> -->
											<!-- 					</td>	 -->
										</tr>
...
// 										},
// 									返回 : function() {
<%-- 										$("#dialog-formD<%=count%>").dialog("close"); --%>
// 										}
// 									},
// 						close : function() {
<%-- 							$("#dialog-formD<%=count%>").dialog("close"); --%>
...
<%-- 						$("#rep_ten<%=count%>").button().on("click", function() { --%>
<%-- 	     					$("#dialog-formD<%=count%>").dialog( "open" ); --%>
<%-- 	  						$("#dialog-formD<%=count%>").find("[type=submit]").hide();}); --%>
					//dialog for取消訂單  - end					
