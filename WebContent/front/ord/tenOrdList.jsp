						...
						<c:forEach var="ordVO" items="${list}" begin="<%=pageIndex%>"
							...
							<tr>
								...
								<td style="text-align: center"><a
									href="javascript:pressesA${s.index}()">${ordVO.ord_no    }</a></td>
								<td style="text-align: center"><a
									href="javascript:pressesB${s.index}()">${rentVO.rent_name}</a></td>
								<td style="text-align: center">${ord_staMap[ordVO.ord_sta]}</td>
								<td style="text-align: center">${tra_staMap[ordVO.tra_mode]}</td>
								<td style="text-align: center" rowspan="2">${ordVO.ten_date       }<br>
									~ <br> ${ordVO.exp_date       }
								</td>
								<td style="text-align: center">${ordVO.ten_days       }天</td>
								<td style="text-align: center">${ordVO.ot_days        }天</td>
								<td style="text-align: center"><a
									href="javascript:pressesC${s.index}()"><%=lesVO.getMname()%></a></td>
...
<%-- 							$("#cc_ord<%=count%>").attr("disabled", true); --%>
				...
				//超連結至該訂單明細
     			function pressesA${s.index}(){
    	 			document.open("<%=request.getContextPath()%>/front/ord/ord.do?ord_no=${ordVO.ord_no}&action=getOne_For_Display", "" ,"height=450,width=450,left=65,top=100,resizable=yes,scrollbars=yes");
     			}
				//超連結至該租物
